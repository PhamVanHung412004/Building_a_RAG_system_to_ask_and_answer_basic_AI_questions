...
file_path_text = Path(__file__).parent.parent / "context"
file_path_dataset = Path(__file__).parent.parent / "dataset"



list_arr = os.listdir(file_path_dataset)




for i in range(len(list_arr)):
    path = file_path_dataset / list_arr[i]
    doc = fitz.open(path)
    string_context = ""
    for page in doc:
        text = page.get_text()
        string_context += text
    file_path_from_context =  str("context_" + str(i + 1) + ".txt")
    with open(file_path_text / file_path_from_context, "w") as file:
        file.write(string_context)  
